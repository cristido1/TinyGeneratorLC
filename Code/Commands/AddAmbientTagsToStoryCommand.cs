...
namespace TinyGenerator.Services.Commands
{
    ...
    public sealed class AddAmbientTagsToStoryCommand : ICommand
    {
        ...
        public async Task<CommandResult> ExecuteAsync(CancellationToken ct = default, string? runId = null)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < chunks.Count; i++)
                {
                    ...
                    var chunk = chunks[i];
                    ReportProgress(effectiveRunId, i + 1, chunks.Count, $"Adding ambient tags chunk {i + 1}/{chunks.Count}");
            ...
            CancellationToken ct)
        {
            ...
            var requestTimeoutSeconds = Math.Max(0, _tuning.AmbientExpert.RequestTimeoutSeconds);
            ...
            for (int attempt = 1; attempt <= maxAttempts; attempt++)
            {
                ...
                ReportProgress(
                    ...
                    chunkIndex,
                    chunkCount,
                    $"Adding ambient tags chunk {chunkIndex}/{chunkCount} (attempt {attempt}/{maxAttempts})");
                ...
                try
                {
                    ...
                    {
                        responseJson = await bridge.CallModelWithToolsAsync(messages, new List<Dictionary<string, object>>(), attemptCts.Token, skipResponseChecker: false);
                    }
                ...
                catch (OperationCanceledException) when (!ct.IsCancellationRequested && requestTimeoutSeconds > 0 && attemptCts.IsCancellationRequested)
                {
                    ...
                    _logger?.Append(
                        runId,
                        $"[chunk {chunkIndex}/{chunkCount}] Request timeout on attempt {attempt}/{maxAttempts} after {requestTimeoutSeconds}s",
                        "warn");
