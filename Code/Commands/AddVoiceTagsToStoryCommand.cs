    ...
    public sealed class AddVoiceTagsToStoryCommand : ICommand
    {
        ...
        public async Task<CommandResult> ExecuteAsync(CancellationToken ct = default, string? runId = null)
        {
            var effectiveRunId = string.IsNullOrWhiteSpace(runId)
                ? (_storiesService?.CurrentDispatcherRunId ?? $"add_voice_tags_to_story_{_storyId}_{DateTime.UtcNow:yyyyMMddHHmmss}")
                : runId;
            ...
            try
            {
                ...
                for (int i = 0; i < chunks.Count; i++)
                {
                    ...
                    var chunk = chunks[i];
                    ReportProgress(effectiveRunId, i + 1, chunks.Count, $"Formatting chunk {i + 1}/{chunks.Count}");
            ...
            CancellationToken ct)
        {
            ...
            for (int attempt = 1; attempt <= maxAttempts; attempt++)
            {
                ...
                try
                {
                    ...
                    {
                        responseJson = await bridge.CallModelWithToolsAsync(messages, new List<Dictionary<string, object>>(), ct, skipResponseChecker: false);
                    }
