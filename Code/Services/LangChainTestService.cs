        ...
        string? testFolder = null)
    {
        ...
        private sealed class QuestionTestCommand : ITestCommand
        {
            ...
            public async Task ExecuteAsync(TestExecutionContext context)
            {
                ...
                fullPrompt = _service.AppendResponseFormatToPrompt(fullPrompt, test.JsonResponseFormat, hasTools, test);
        ...
        private string AppendResponseFormatToPrompt(string prompt, string? responseFormat, bool hasTools = false, TestDefinition? test = null)
        {
            if (string.IsNullOrWhiteSpace(responseFormat))
                return prompt;

            // Text-eval tests intentionally require plain-text numeric output (no JSON wrapper).
            if (string.Equals(test?.GroupName, "texteval", StringComparison.OrdinalIgnoreCase))
                return prompt;
