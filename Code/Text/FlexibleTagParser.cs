...
public static class FlexibleTagParser
{
    ...
    public static bool TryParseTagHeaderLine(string line, out string tag, out string value)
    {
        ...
        // TAG - value (hyphen only when surrounded by spaces, to avoid parsing values like PG-13 as a tag line)
        var hyphenSep = " - ";
        var hyphenIdx = candidate.IndexOf(hyphenSep, StringComparison.Ordinal);
        if (hyphenIdx > 0)
        {
            var header = candidate[..hyphenIdx].Trim();
            if (IsLikelyTagName(header))
            {
                tag = header;
                value = NormalizeValue(candidate[(hyphenIdx + hyphenSep.Length)..]);
                return true;
            }
        }
