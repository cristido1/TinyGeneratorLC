            ...
            for (int attempt = 1; attempt <= MaxAttemptsPerChunk; attempt++)
            {
                ...
                try
                {
                    ...
                    if (cleaned.Length < inputLength)
                    {
                        ...
                        lastError = "Il testo ritornato è più corto dell'originale. NON devi tagliare o rimuovere testo, devi solo aggiungere i tag [FX:] mantenendo tutto il contenuto originale.";
                        continue;
                    }

                    // Validation 2: Check for minimum tag count (at least 3 FX tags)
                    var tagCount = System.Text.RegularExpressions.Regex.Matches(cleaned, @"\[FX:", System.Text.RegularExpressions.RegexOptions.IgnoreCase).Count;
                    if (tagCount < 3)
                    {
                        _logger?.Append(runId, $"[chunk {chunkIndex}/{chunkCount}] Not enough FX tags: {tagCount} found, minimum 3 required", "warn");
                        lastError = $"Hai inserito solo {tagCount} tag [FX:]. Devi inserire ALMENO 3 effetti sonori per rendere la scena più vivida e realistica.";
                        continue;
                    }

                    // All validations passed
                    _logger?.Append(runId, $"[chunk {chunkIndex}/{chunkCount}] Validated: {cleaned.Length} chars, {tagCount} FX tags");
                    return cleaned;
                }
                catch (Exception ex)
                {
