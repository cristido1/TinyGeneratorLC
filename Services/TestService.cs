...
namespace TinyGenerator.Services
{
    ...
    public class TestService : ITestService
    {
        ...
        public TestService(DatabaseService database, ProgressService progress)
        {
        ...
        public async Task ExecuteTestAsync(int runId, int idx, TestDefinition t, KernelFactory factory, string model, ModelInfo modelInfo, string agentInstructions, ChatCompletionAgent? defaultAgent)
        {
            ...
            try
            {
                ...
                if (string.Equals(t.TestType, "question", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    _database.UpdateTestStepResult(stepId, passedQ, outJsonQ, passedQ ? null : failReasonQ, (long?)(swStep.Elapsed.TotalMilliseconds));
                ...
                // --- TEST TYPE: FUNCTIONCALL ---
                var stepAllowed = (t.AllowedPlugins ?? string.Empty).Split(',').Select(s => (s ?? string.Empty).Trim().ToLowerInvariant()).Where(s => !string.IsNullOrWhiteSpace(s)).ToArray();
                if (stepAllowed.Length == 0)
                {
                    stepAllowed = new[] { (t.Library ?? "text").ToLowerInvariant() };
                }
                ChatCompletionAgent? stepAgent = null;
                try
                {
                    var stepKernel = factory.CreateKernel(model, stepAllowed);
                ...
                try
                {
                    called = (t.Library ?? string.Empty).ToLowerInvariant() switch
                    {
                ...
                // For evaluator tests: require FunctionCalls. Extract score/defects from resultObj.FunctionCalls
                var libLower = (t.Library ?? string.Empty).ToLowerInvariant();
                var isTexteval = string.Equals(t.GroupName, "texteval", StringComparison.OrdinalIgnoreCase);
                ...
                if (isTexteval || evaluatorLibsLocal.Contains(libLower))
                {
                    ...
                    if (!passedEval && parsedOk && !string.IsNullOrWhiteSpace(t.ValidScoreRange))
                    {
                        try
                        {
                            ...
                            {
                                var values = range.Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
                                // Prova a confrontare sia score che defects
                                var scoreStr = parsedScore.ToString();
                                foreach (var v in values)
                                {
                                    if (scoreStr.Equals(v, StringComparison.OrdinalIgnoreCase) ||
                                        (!string.IsNullOrWhiteSpace(parsedDefects) && parsedDefects.Equals(v, StringComparison.OrdinalIgnoreCase)))
                                    {
                                        passedEval = true;
                                        break;
                                    }
                ...
                if (!string.IsNullOrWhiteSpace(t.ExpectedBehavior) && t.ExpectedBehavior.Contains("LastCalled") && string.IsNullOrWhiteSpace(called))
                {
                ...
                if (!string.IsNullOrWhiteSpace(t.ExpectedBehavior) && t.ExpectedBehavior.Contains("LastCalled") && !string.IsNullOrWhiteSpace(called))
                {
                    ...
                    passed = string.Equals(expected, called, StringComparison.OrdinalIgnoreCase);
                }
