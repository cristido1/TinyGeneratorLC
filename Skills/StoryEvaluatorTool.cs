    ...
    public class StoryEvaluatorTool : BaseLangChainTool, ITinyTool
    {
            ...
            return CreateFunctionSchema(
                ...
                new Dictionary<string, object>
                {
                    ...
                    },
                    {
                        ...
                        {
                            { "type", "integer" },
                            { "description", "Story ID for persistence" }
                        }
                    }
                },
        ...
        public override async Task<string> ExecuteAsync(string input)
        {
            try
            {
                ...
                return request.Operation?.ToLowerInvariant() switch
                {
                    ...
                    "describe" => SerializeResult(new { result = "Available operations: evaluate_single_feature(score, defects, feature, story_id), evaluate_full_story(...all categories..., story_id)." }),
        ...
        private string ExecuteEvaluateSingleFeature(StoryEvaluatorToolRequest request)
        {
            try
            {
                ...
                if (_db != null && request.StoryId > 0)
                {
                    try
                    {
                        var json = LastResult!;
                        var total = finalScore;
                        _db.AddStoryEvaluation(request.StoryId, json, total, ModelId, AgentId);
                    }
                    catch { }
                }
